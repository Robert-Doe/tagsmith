 ...
 * exception: a legacy no-semicolon named match is undone entirely -- as
 * if it were never attempted -- if it's immediately followed by '=' or
 * an alphanumeric character, but ONLY when consumed as part of an
 * attribute value. Outside attributes (Data/RCDATA, Module 4), the exact
 * same input decodes normally; this is a real, deliberate asymmetry the
 * spec draws for historical reasons (protecting things like
 * `href="?a=1&amp=2"` from being wrongly decoded). See DECISIONS.md.
 */
function consumeNamedReference(tokenizer, startPos, isInAttribute) {
 ...
 * additionalAllowedCharacter and isInAttribute are used from attribute-
 * value context (Module 11) -- Data/RCDATA (Module 4) always pass
 * undefined/false.
 */
function consumeCharacterReference(tokenizer, additionalAllowedCharacter, isInAttribute = false) {
