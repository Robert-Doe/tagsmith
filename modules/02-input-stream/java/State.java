/**
 * Every named state from HTML5 tokenization spec section 8.2.4. 68 states
 * total (the spec's 69th subsection, "Tokenizing character references," is
 * a shared subroutine invoked BY states like CHARACTER_REFERENCE_IN_DATA —
 * it is not itself a switch-target state).
 *
 * Each constant carries the exact spec section title (minus the trailing
 * "state") in specName, so an error message that names a state doubles as
 * a search string you can paste into the spec page.
 */
public enum State {
    DATA("Data"),
