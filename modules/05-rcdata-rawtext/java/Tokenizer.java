...
public class Tokenizer {
    ...
    /** Shared by RCDATA/RAWTEXT less-than-sign states: '/' commits to
     * trying an end tag; anything else is just a literal '<', with the
     * current character reconsumed (never actually consumed here, only
     * peeked). */
    private void lessThanSignState(State endTagOpenState, State returnState) {
        Character next = peek();
        if (next != null && next == '/') {
            tempBuffer = "";
            consume();
    ...
    private void rcdataLessThanSignState() {
        lessThanSignState(State.RCDATA_END_TAG_OPEN, State.RCDATA);
    }

    private void rawtextLessThanSignState() {
        lessThanSignState(State.RAWTEXT_END_TAG_OPEN, State.RAWTEXT);
    }
